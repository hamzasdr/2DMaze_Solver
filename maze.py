...
def solve_maze(game,coordinates,wall):
   ...
   time.sleep(0.5)
   for i, j in solution:
       pygame.draw.rect(game.window, RED, [102 + i * space, 102 + j * space, space - 2, space - 2], 0)
       pygame.display.flip()
       clock.tick(15)
       pygame.draw.rect(game.window, LRED, [102 + i * space, 102 + j * space, space - 2, space - 2], 0)
       pygame.display.flip()
...
def A_star(wall,src,dist,size):
    ...
    while not open.empty():
        ...
        for direction in range(0, 4):
            ...
            if (not closed[temp_i][temp_j] and not find_element(open,(temp_i,temp_j))) or cells[temp_i][temp_j].f > temp_f:
